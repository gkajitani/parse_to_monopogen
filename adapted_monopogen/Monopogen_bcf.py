...
def germline(args):
	...
	with open(args.region) as f_in:
		for line in f_in:
			...
			cmd1 = bcftools + " mpileup -b " + bam_filter + " -f "  + args.reference  + " -r " +  jobid + " -q 20 -Q 20 --threads 40 --incl-flags 0 --excl-flags 0 -a FORMAT/AD  -d 10000000 "
			...
			cmd5 = java + " -Xmx20g -jar " + beagle +  " gt=" +  out + "/germline/" +  jobid + ".germline.vcf"  +  " ref=" +  imputation_vcf    +  "  chrom=" + record[0]  + " out="   +  out + "/germline/" + jobid+ ".phased " + "impute=false  modelscale=2  nthreads=20  gprobs=true  niterations=0"
